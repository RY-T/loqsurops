...
rule all:
    input:
        expand('RNA_LIB/02preprocessouput/{LIB}_final_col_filter.fasta', LIB=lib_list)

rule clip_convert_to_fasta_and_identifiers:
    input:
        'RNA_LIB/{LIB}.fastq'
    output:
        'RNA_LIB/{LIB}clip.fasta'
    shell:"""
...
rule collapsing_repeats:
    input:
        'RNA_LIB/{LIB}clip.fasta'
    output:
        'RNA_LIB/{LIB}clipcolid_unlabeled.fasta'
    shell:"""
...
rule filter_length:
    input:
        intab = 'RNA_LIB/{LIB}.tab'
    output:
        outtab = 'RNA_LIB/{LIB}len.tab'
    run:
        import pandas as pd
...
rule convert_to_fasta:
    input:
        intab = 'RNA_LIB/{LIB}len.tab'
    output:
        outfasta = 'RNA_LIB/{LIB}len.fasta'
    run:
        import pandas as pd
        import re
...
rule artifact_filter:
    input:
        'RNA_LIB/{LIB}len.fasta'
    output:
        'RNA_LIB/{LIB}_final_col_filter.fasta'
    shell:"""
...
rule moving_files:
    input:
        'RNA_LIB/{LIB}_final_col_filter.fasta'
    output:
        'RNA_LIB/02preprocessouput/{LIB}_final_col_filter.fasta'
    shell:"""
